...
############################### For MCMC ###############################
def generate_model_spectrum_mcmc_one_pop(x_all):
    """
    For MCMC; single dust population
    model parameters:
    [0, 1,   2,    3,      4,   5,   ... ...  ]
    [f, p, a_min, a_max,  f_B, f_C,  ... ...  ]
    """
    # dust population # 1
    ...
    # number of components in the dust population #1 starts in n_parameters - 4
    # dust population #1
    N = n_parameters - 4 
    porosity = 1- sum(x_all[n+4] for n in np.arange(0, N))
    if  porosity <0: 
        return I
    vf = np.concatenate((np.array([porosity]), np.array([x_all[n+4] for n in np.arange(0, N) ])), axis =0)
    ...
    f_I = interp1d(wave_inter, I_nz_iter)
...
def lnpost_mcmc(x_all):
    ...
    model = generate_model_spectrum_mcmc_one_pop(x_all) 
...
############################### end for MCMC ############################### 
############## output plots  #########
def output_best_fit_mcmc_result_one_pop(samples):
    """
    sigle dust population
    model parameters:
    [0, 1,   2,    3,      4,   5,   ... ...  ]
    [f, p, a_min, a_max,  f_B, f_C,  ... ...  ]
    """
    # dust population # 1
    if n_parameters_pop1 == 2:
        scaling_factor, p, a_min, a_max, f_B, f_C,  = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0)))   
        porosity = np.ones((3)) - f_B - f_C  

        mcmc_values = np.array([scaling_factor, p, a_min, a_max,  porosity, f_B, f_C, ])

        best_fit_para = [scaling_factor[0], p[0], a_min[0], a_max[0],   f_B[0], f_C[0],   ] 

        model_bestfit = generate_model_spectrum_mcmc_one_pop(best_fit_para) 
        ...
        labels = [r'$f$', r'$p$', r'$a_{min}$', r'$a_{max}$', 
    ...
    elif n_parameters_pop1 == 3:
        scaling_factor, p, a_min, a_max,  f_B, f_C, f_D,   = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0)))    
        porosity = np.ones((3)) - f_B - f_C -f_D

        mcmc_values = np.array([scaling_factor, p, a_min, a_max,  porosity, f_B, f_C, f_D, ])

        best_fit_para = [scaling_factor[0], p[0], a_min[0], a_max[0],    f_B[0], f_C[0], f_D[0],   ]  
    ...
    elif n_parameters_pop1 == 4:
        scaling_factor, p, a_min, a_max,  f_B, f_C, f_D, f_E,  = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0)))    
        porosity = np.ones((3)) - f_B - f_C - f_D - f_E
        mcmc_values = np.array([scaling_factor, p, a_min, a_max, f_B, f_C, f_D, f_E,  ])

        best_fit_para = [scaling_factor[0], p[0], a_min[0], a_max[0],   f_B[0], f_C[0], f_D[0], f_E[0],  ]  
    ...
    elif n_parameters_pop1 == 5:
        scaling_factor, p, a_min, a_max,  f_B, f_C, f_D, f_E, f_F,   = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0)))    
        porosity = np.ones((3)) - f_B - f_C - f_D - f_E - f_F

        mcmc_values = np.array([scaling_factor, p, a_min, a_max,  porosity, f_B, f_C, f_D, f_E, f_F, ])

        best_fit_para = [scaling_factor[0], p[0], a_min[0], a_max[0],    f_B[0], f_C[0], f_D[0], f_E[0], f_F[0],   ]  
    ...
    elif n_parameters_pop1 == 6:
        scaling_factor, p, a_min, a_max,  f_B, f_C, f_D, f_E, f_F, f_G, = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [16, 50, 84], axis=0)))    
        porosity = np.ones((3)) - f_B - f_C - f_D - f_E - f_F - f_G

        mcmc_values = np.array([scaling_factor, p, a_min, a_max,  porosity, f_B, f_C, f_D, f_E, f_F, f_G,])

        best_fit_para = [scaling_factor[0], p[0], a_min[0], a_max[0],    f_B[0], f_C[0], f_D[0], f_E[0], f_F[0], f_G[0], ]  
...
if n_parameters_pop1 == 2 and n_parameters_pop2 ==0:
    x_all = [1, -3.6, 1.6, 100,   0.1, 0.05,   ] 
    bounds = np.array([[0, -4.5, 0, 20,    0, 0,   ],[1, -2, 20, 1000,  1, 1,   ]])
...
elif n_parameters_pop1 == 3 and n_parameters_pop2 ==0:
    x_all = [1, -3.6, 1.6, 100,   0.05, 0.05, 0.5,  ] 
    bounds = np.array([[0, -5, 0.02, 20,   0, 0, 0, 0, ],[1, -2, 20, 1000,   1, 1, 1, 1,    ]])
...
elif n_parameters_pop1 == 4 and n_parameters_pop2 ==0:
    x_all = [2, -3.5, 1.6, 200,   0.01, 0.01, 0.2, 0.5,  ] 
    bounds = np.array([[0, -5, 0.02, 20,   0, 0, 0, 0,  ],[1, -2, 20, 1000,   1, 1, 1, 1,   ]])
...
elif n_parameters_pop1 == 5 and n_parameters_pop2 ==0:
    x_all = [1, -3.6, 1.6, 100,   0.05, 0.05, 0.1, 0.6, 0.1]  
    bounds = np.array([[0, -5, 0.02, 20,   0, 0, 0, 0, 0 ],[1, -2, 20, 1000,   1, 1, 1, 1, 1   ]])
...
elif n_parameters_pop1 == 6 and n_parameters_pop2 ==0:
    x_all = [1, -3.6, 1.6, 100,   0.05, 0.05, 0.1, 0.5, 0.1, 0.1]  
    bounds = np.array([[0, -5, 0.02, 20,   0, 0, 0, 0, 0, 0 ],[1, -2, 20, 1000,   1, 1, 1, 1, 1, 1  ]])
...
#################################################################################
#######     Start spec fitting;     mcmc;    single dust population     #########
#################################################################################
...
model_bestfit, mcmc_values, chi2_reduced, labels = output_best_fit_mcmc_result_one_pop(samples) 
