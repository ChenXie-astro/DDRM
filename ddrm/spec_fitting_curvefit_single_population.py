...
def generate_model_spectrum_mcmc_one_pop(_, *x_all):
    """
    For 'curve_fit'
    model parameters:
    [0, 1,   2,    3,      4,   5,   ... ...  ]
...
def output_best_fit_result(x_all):
    ...
    model parameters:
    [0, 1,   2,    3,    4,    5,       6,     7,   8,   ... ...  ]
    [f, p, a_min, a_max, p2, a_min2, a_max2,  f_B, f_C,  ... ...  ]
    """
    # dust population # 1
    if n_parameters_pop1 == 2 :
        ...
        model_bestfit = generate_model_spectrum_mcmc_one_pop(z_wave, *x_all) 
    ...
    color ='#EA8379'
    # plotname = '/new_mie_SB_{0}_{1}_{2}_{3}_{4}'.format(round(a_min,2), round(a_max,2), round(p,3), round(chi2_reduced,3), spec_region) 
...
num_integ = 500     # sampling in calculating the model reflectance at a given wavelength; 500 is sufficient0
...
if n_parameters_pop1 == 2 and n_parameters_pop2 ==0:
    x_all = [1, -3.6, 1.6, 100,   0.1, 0.05,   ] # inner
    ...
    x_all = [1, -3.2, 0.9, 75,   0.18, 0.001, 0.35,  ] # inner+mid
    bounds = np.array([[0, -5, 0.02, 20,   0, 0, 0,  ],[1, -2, 20, 1000,   1, 1, 1,     ]])
    ...
    x_all = [1, -3.5, 1.5, 500,   0.01, 0.01, 0.1, 0.7,  ] # inner
    ...
    x_all = [1, -3.6, 1.6, 100,   0.05, 0.05, 0.1, 0.6, 0.1]  # inner
    ...
    x_all = [1, -3.6, 1.6, 100,   0.05, 0.05, 0.1, 0.5, 0.1, 0.1]  # inner
...
popt_inital, pcov = curve_fit(generate_model_spectrum_mcmc_one_pop, x,y, expected, sigma=sigma, absolute_sigma=True, bounds=bounds,  maxfev=5000)
popt, pcov = curve_fit(generate_model_spectrum_mcmc_one_pop, x, y, popt_inital, sigma=sigma, absolute_sigma=True, bounds=bounds,  maxfev=50000)
